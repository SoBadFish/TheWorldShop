package org.badfish.theworldshop.db;
...
//        return  set(tableName,key,value,sqlData);
    ...
    public <T> List<T> getColumnAllData(String tableName,String column, Class<T> clazz){
        ArrayList<T> datas = new ArrayList<>();

        try {
            // 准备 SQL 查询语句
            String query = "SELECT "+column+" FROM " + tableName +" GROUP BY "+column;
            PreparedStatement statement = connection.prepareStatement(query);
    ...
    public <T> LinkedList<T> getDataByString(String tableName, String selection, String[] key,int maxSize,int page, Class<T> clazz) {
        LinkedList<T> datas = new LinkedList<>();
        try {
            // 准备 SQL 查询语句
            String query = "SELECT * FROM " + tableName + " WHERE " + selection+" LIMIT "+maxSize+" "+(page - 1);
            PreparedStatement statement = connection.prepareStatement(query);
